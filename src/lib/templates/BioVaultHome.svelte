<script lang="ts">
	...
	const pins = $derived(banks.flatMap((b: any) =>
...
<section class="relative mb-8 overflow-hidden rounded-[var(--radius)] border">
	...
	<div class="relative grid gap-6 p-6 sm:p-9 lg:grid-cols-[1.05fr_1fr] lg:items-center">
		<div>
			...
			<div class="mt-5 flex flex-wrap gap-2">
				<a href="/explore" class="brand-gradient rounded-md px-4 py-2 text-sm font-semibold text-white shadow-sm">Explore variants</a>
				<a href="/api/biobanks" class="rounded-md border px-4 py-2 text-sm font-medium hover:bg-muted">View the API</a>
			</div>
		</div>
		<GeoMap {pins} center={tenant.map.center} zoom={tenant.map.zoom} onselect={(p) => (location.href = go(p.biobankSlug))} />
	</div>
</section>
...
<div class="mb-9 grid gap-3 sm:grid-cols-2">
	{#each banks as b}
		...
		<a href={go(b.slug)} class="card-surface group flex items-center gap-4 p-4 transition hover:shadow-md">
			<span class="grid size-12 shrink-0 place-items-center rounded-xl text-2xl" style={`background:color-mix(in oklch, ${t?.theme.primary} 16%, transparent)`}>{t?.logoEmoji ?? '🧬'}</span>
