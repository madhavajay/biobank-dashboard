<script lang="ts">
	...
	type TableTabId = 'populations' | 'gnomad' | 'clinvar' | 'clinpgx' | 'genes' | 'vrs';
	let activeTableTab = $state<TableTabId>('populations');
	const useTableTabs = $derived(part === 'tables');
	const tableTabs = $derived.by(() => {
		const tabs: { id: TableTabId; label: string }[] = [
			{ id: 'populations', label: 'Populations' },
			{ id: 'gnomad', label: 'gnomAD' },
			{ id: 'clinvar', label: 'ClinVar' },
			{ id: 'clinpgx', label: 'ClinPGx' },
			{ id: 'genes', label: 'Genes' }
		];
		if (vrsJson) tabs.push({ id: 'vrs', label: 'VRS' });
		return tabs;
	});
			...
			<dd class="mt-1">
				{#if geneSymbols.length}
					<div class="flex flex-wrap gap-1">
						{#each geneSymbols as symbol}
							<a href={geneHref(symbol)} onclick={(event) => openExploreLink(geneHref(symbol), event)} class="rounded border px-1.5 py-0.5 text-xs font-medium text-primary hover:bg-muted">{symbol}</a>
						{/each}
					</div>
				{:else}
			...
			<dd class="mt-1 min-w-0">
				{#if v.vepLabel}
					<a
						...
						onclick={(event) => openExploreLink(explorerFilterHref('vepConsequence', v.vepLabel), event)}
			...
			<dd class="mt-1">
				{#if v.vepImpact}
					<a
						...
						onclick={(event) => openExploreLink(explorerFilterHref('vepImpact', v.vepImpact), event)}
...
{#if showTables}
	...
	<section
		class="card-surface relative z-[80] overflow-visible"
		class:mb-5={!useTableTabs}
					...
					{#each sortedClinvarRows as row}
						<tr class="border-t">
							...
							<td class="px-3 py-2 font-medium">
								<a href={geneHref(row.gene)} onclick={(event) => openExploreLink(geneHref(row.gene), event)} class="text-primary hover:underline">{row.gene || '-'}</a>
							</td>
	...
	<section
		class="card-surface relative z-[70] overflow-visible"
		class:mb-5={!useTableTabs}
			...
			<table class="w-full text-sm">
				...
				<tbody>
					{#each v.genes as g}
						<tr class="border-t">
							<td class="px-3 py-2 font-medium">
								<a href={geneHref(g.symbol)} onclick={(event) => openExploreLink(geneHref(g.symbol), event)} class="text-primary hover:underline">{g.symbol}</a>
							</td>
...
<style>
	.variant-detail-tables:not(.is-tabbed),
	.variant-detail-tab-panels:not(.is-tabbed) {
		display: contents;
	}
</style>
