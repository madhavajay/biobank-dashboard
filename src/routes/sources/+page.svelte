...
<div class="mb-5 flex flex-wrap items-end justify-between gap-4 border-b pb-5">
	<div>
		<h1 class="text-3xl font-bold tracking-tight">Biobanks</h1>
...
<div class="grid gap-5 lg:grid-cols-2">
	{#each data.sources as source}
		...
		<Card.Root class="gap-0 overflow-hidden py-0">
			<div class="flex items-center justify-center border-b bg-muted/25 px-6 pb-5 pt-3">
				{#if tenant?.logoImg}
					<img
						src={tenant.logoImg}
						alt=""
						class="block max-h-24 w-auto max-w-[min(100%,16rem)] object-contain"
					/>
				{:else}
			...
			<Card.Content class="flex flex-col gap-4 pt-5">
				<div class="flex items-start justify-between gap-3">
					...
					{#if portalUrl}
						<Button href={portalUrl} target="_blank" rel="noreferrer" size="sm" class="shrink-0">
							Go to Portal
						</Button>
					{/if}
				</div>
