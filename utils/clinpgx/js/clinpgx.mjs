...
export function normalizeVariantRow(raw, site = DEFAULT_SITE) {
	const locations = (raw.locations || []).map(normalizeLocation);
	const relatedGenes = (raw.relatedGenes || []).map((g) => ({
		id: g.id,
		symbol: g.symbol,
		name: g.name || ''
	}));
...
export function normalizeGeneRow(raw, site = DEFAULT_SITE) {
	return {
		id: raw.id,
		symbol: raw.symbol || '',
		name: raw.name || '',
...
export function normalizeClinicalAnnotationRow(raw, site = DEFAULT_SITE) {
	return {
		id: raw.id,
